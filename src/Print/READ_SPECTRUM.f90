...
USE WAM_OUTPUT_SET_UP_MODULE,ONLY: PFLAG
...
READ (IUNIT, END=4000) PFLAG(31:34)
KL = NINT(XANG)
ML = NINT(XFRE)
...
IF (.NOT.ALLOCATED(THETA)) THEN
   ...
   END DO
END IF
IF (PFLAG(31) .AND. .NOT.ALLOCATED(SPEC) )      ALLOCATE (SPEC(1:KL,1:ML))
IF (PFLAG(32) .AND. .NOT.ALLOCATED(SPEC_SEA)  ) ALLOCATE (SPEC_SEA(1:KL,1:ML))
IF (PFLAG(33) .AND. .NOT.ALLOCATED(SPEC_SWELL)) ALLOCATE (SPEC_SWELL(1:KL,1:ML))
...
IF (PFLAG(31)) THEN
   READ(IUNIT) U10, UDIR, US, HS, PPER, MPER, TM1, TM2, MDIR, SPRE
   READ(IUNIT) SPEC
END IF
IF (PFLAG(32)) THEN
   READ(IUNIT) U10, UDIR, US, HS_SEA, PPER_SEA, MPER_SEA, TM1_SEA, TM2_SEA,    &
&              MDIR_SEA, SPRE_SEA
   READ(IUNIT) SPEC_SEA
END IF
IF (PFLAG(33)) THEN
   READ(IUNIT) U10, UDIR, US, HS_SWELL, PPER_SWELL, MPER_SWELL, TM1_SWELL,     &
&              TM2_SWELL, MDIR_SWELL, SPRE_SWELL
